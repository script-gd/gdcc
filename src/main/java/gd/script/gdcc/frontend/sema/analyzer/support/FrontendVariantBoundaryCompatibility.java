...
public final class FrontendVariantBoundaryCompatibility {
    ...
    public static @NotNull Decision determineFrontendBoundaryDecision(
        ...
        return switch (source) {
            ...
            case GdIntType _ when target instanceof GdFloatType -> Decision.ALLOW_WITH_INTRINSIC_CAST;
    ...
    private static boolean isSupportedSameDimensionVectorWidening(
            @NotNull GdIntVectorType source,
            @NotNull GdFloatVectorType target
    ) {
        var dimension = source.getDimension();
        return dimension == target.getDimension() && switch (dimension) {
            case 2, 3, 4 -> true;
            default -> false;
        };
    }
