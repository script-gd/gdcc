...
# gdcc-test: output_not_contains=frontend typed dictionary property plain guard after bad set.
extends Node
...
func _ready() -> void:
    ...
    if int(target.call("read_payload_size")) != 1:
        push_error("Typed dictionary property plain-guard setup failed.")
        return
    ...
    print("frontend typed dictionary property plain guard after bad set.")
