class_name BenchmarkSlidingVarianceInterpreter
extends Node
...
func benchmark() -> int:
    var index := _window_size
    var update_index := 0
    while update_index < _update_count:
        var incoming := _sample_value(index)
        var outgoing := _sample_value(index - _window_size)
...
func _sample_value(index: int) -> int:
    var residue := ((index * index) + (3 * index) + 7) % 13
...
func _expected_scaled_variance() -> int:
    ...
    var update_index := 0
    var next_index := _window_size
    while update_index < _update_count:
        var incoming := _sample_value(next_index)
        var outgoing := _sample_value(next_index - _window_size)
