...
- `gdcc_coro_cancel_all()`: HR-5 bulk abandonment for hot reload, called from the
  ...
  `gdextension_interface.cpp` `default_compare_equal`) purely as a lookup key — its free
  callback is NULL because the connection's own handle releases the wait exactly once.
