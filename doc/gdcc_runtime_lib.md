...
- `gdcc_coro_state_header`: the common frame header embedded in every hidden state class
  ...
  - `reloaded_shell` flag: RELOADED_SHELL terminal state set only by the generated
    ...
    (`hot_reload_implementation_plan.md` D5). Mutually exclusive with running any body:
  ...
  - `signal_reg`: HR-5 registration of an in-flight one-shot signal wait (opaque
...
- `gdcc_coro_state_free(state)`: the generic half of the state class `free_instance` —
  ...
  Also defensively unlinks the state from the HR-5 active list (finalize/cancel normally
  already did). The
...
- `gdcc_coro_await_signal(godot_Signal *sig, godot_Variant *out, mco_coro *co,
  ...
  return **without** suspending. HR-5 registration contract: after a successful connect the
...
- `gdcc_coro_cancel(gdcc_coro_state_header *state)`: the abandonment path, mutually
  ...
  fields. Cancel also unlinks the state from the HR-5 active list (idempotent), which is
  what makes the nested-PREDELETE cascades triggered by its own waiter-edge releases safe
  during a bulk cancel.
- `gdcc_coro_active_link(state)`: HR-5 active-list insertion, called by the generated
...
- `gdcc_coro_set_hot_reload_active(bool)`: HR-5 dual-mode gate, module-global, default
  ...
  set it), every HR-5 tracking facility is bypassed: `gdcc_coro_active_link` and
  `gdcc_coro_cancel_all` are no-ops and no per-await signal registration is allocated, so
  coroutines behave exactly as before HR-5 with zero tracking cost. The ordinary
  lifecycle (await/finalize/PREDELETE cancel) is unaffected either way.
- `gdcc_coro_cancel_all()`: HR-5 bulk abandonment for hot reload, called from the
  generated `deinitialize()` before static teardown and class unregistration (D8 v11), so
...
This section freezes the HRX runtime contract (design source:
`module_impl/backend/hot_reload_implementation_plan.md` §5; user-visible semantics are
summarized there §7). It exists so a custom Callable held by Godot never stores a library
code address: in editor processes every `call`/`free`/`is_valid`/`get_argument_count`
pointer is a thunk emitted into executable heap memory owned by no library image, so the
