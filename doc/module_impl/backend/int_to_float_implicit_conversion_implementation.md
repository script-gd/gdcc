...
- exact match 高于 primitive cast
- primitive cast 高于 `Variant` pack/unpack
- rejected pair 不参与 successful overload selection

### Frontend lowering materialization

- 入口：`FrontendBodyLoweringSession.materializeFrontendBoundaryValue(...)`
- `ALLOW_WITH_PRIMITIVE_CAST` 当前只允许 `GdIntType.INT -> GdFloatType.FLOAT`
...
- `ALLOW_WITH_PRIMITIVE_CAST` 不得作为 `ALLOW_DIRECT` 的别名
...
- destroyable/object result 的 intrinsic 必须单独审计 ownership，不得照抄 primitive cast 的 direct expression route
