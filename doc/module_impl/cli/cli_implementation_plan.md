...
- `-o` / `--output` 可选指定编译目标，语义与 clang 风格一致：多个输入文件对应一个输出目标；省略时由输入文件名推导默认目标。
...
### 1.2 `-o` / `--output` 与默认 output 解释规则

省略 `-o` / `--output` 时，CLI 从已通过输入校验的 host file names 推导一个默认 output target：

- 对每个输入文件取 host file name。
- 去掉末尾 `.gd` 后缀。
- 按命令行输入顺序用 `_` 连接。
- 将连接后的字符串当作无父路径 output target，再套用下面同一套 path 映射规则。
...
- `gdcc a.gd b.gd` 创建一个名为 `a_b` 的 API module，宿主机构建目录为当前工作目录下的 `a_b`。
- generated C 文件和最终 artifact 产出到 `build/demo` 下。
- CLI 从 `CompileResult.artifacts()` 报告 artifact path，而不是自行猜测。
- 显式 blank output path、root-only path，以及被 `API.createModule(...)` 拒绝的 module name，以 exit code `2` 失败。
...
- `GdccCommandOptionTest` 覆盖声明式 annotations、help 文本、缺失必填 input 时 picocli 在 command body 前失败、optional `-o/--output` 解析、repeatable `--class-map`、`-vv` 计数，以及 Step 1 临时边界。
...
- `-o/--output` 不再是 picocli 必填项；缺省值在 command body 内根据已校验输入文件名推导。
...
- [x] 要求至少一个文件。
- [x] 支持省略 `-o/--output`，并从输入 `.gd` 文件名推导默认 output target。
...
- `GdccCommand.call()` 现在先预读所有输入文件，再解析显式 `-o/--output` 或由输入 file stems 推导默认 output target，
  最后才调用 `API.createModule(...)` 和 `API.putFile(...)`。因此缺失文件、目录输入、显式 blank/root-only output path 等错误不会创建
  module。
...
- 新增 `GdccCommandInputTest`，覆盖 UTF-8 source 读取、默认 output target、同名 basename、重复 host file、缺失文件、目录输入、blank/root-only
...
- [x] `projectPath` 来自显式 `-o/--output` path，省略时来自默认 output target。
...
- 显式 `-o/--output` 继续使用最后一个 path segment 作为 `moduleId`/`moduleName`；`CompileOptions.projectPath` 使用完整输出路径。
  若用户传入无父路径的输出目标，例如 `-o demo`，则 project path 使用当前工作目录下的 `demo` 目录。省略 `-o/--output`
  时，默认 target 同样按无父路径处理。
...
- [x] `gdcc a.gd b.gd` 默认创建名为 `a_b` 的 API module。
- [x] 除 `Main` 外，新增 CLI 生产代码全部位于 `dev.superice.gdcc.cli` 包。
...
- [x] `-o/--output` 控制 module name 和宿主机构建目录；省略时由输入文件名推导。
