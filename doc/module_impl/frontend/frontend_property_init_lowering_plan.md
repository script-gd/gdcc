...
- constructor-time object-valued property apply 通过 `CBodyBuilder#applyPropertyInitializerFirstWrite(...)` 复用统一 ptr-conversion / ownership-consume 语义，同时保持 first-write 不释放旧值
- object-valued init helper 若通过本地 owning slot 返回值，return path 会把该 slot move 到 `_return_val`，避免 `__finally__` 在 caller 消费前提前销毁返回对象
...
- object-valued init helper 返回本地 owning slot 时，不再被 `__finally__` auto-destroy 提前释放。
