...
- 明确非目标：
  - `CastExpression` / `as` 由独立合同 `frontend_cast_expression_implementation.md` 管理；`is_instance_of()` 全局函数和 `not in` 不属于本合同（`not in` 由 `frontend_not_in_operator_plan.md` 的复合规则承接）
