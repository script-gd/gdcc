...
- 明确非目标：
  ...
  - 不在这里为任何已识别表达式补 lowering（`PreloadExpression`、`GetNodeExpression`、`ArrayExpression` / `DictionaryExpression` / `ConditionalExpression` 等均已 compile-ready，lowering 合同见各自事实源文档）
...
这条边界的目的不是“少报错”，而是避免 compile gate 把已经被上游明确封口的恢复域重新打平成 lowering surface。`ForStatement` 不属于该跳过集合：它已进入 shared semantic，compile gate 会按 route-aware policy 处理——已注册 lowering contract 的 route 会命中 statement root 并进入 body 重扫 facts，未注册 contract 的 route 则在 statement root 发 route-not-ready blocker。`MatchStatement` 同样不属于该跳过集合：compile gate 读取 `matchPlans()`，`WILDCARD` / `BINDING` / `LITERAL` / `EXPRESSION` / `ARRAY` / `DICTIONARY` 六 route 全部放行并重扫 facts。
...
`AssertStatement` 已从显式封口列表移除：compile gate 对 `assert` 只做 `markCompileSurfaceNode` 并 `walkExpression` 其 condition 与可选 message，不再发出 `sema.compile_check`。

保持不变的两条事实：

- frontend 已经识别并正常遍历 `assert`
- shared type-check 继续把 `assert` condition 当成普通 source condition 处理（Godot-compatible truthiness，不回退为 strict-bool）；message 的 String 可赋值校验由 shared type-check 持有（见 `frontend_type_check_analyzer_implementation.md` §3.6）
...
当前没有表达式级显式 compile 拦截；所有已识别表达式均由 shared semantic published facts + generic published-fact scan 判定。

`GetNodeExpression` 不属于显式 compile-block 列表：shared semantic 在 Node 派生类非 static 函数体及已记录 lambda 中发布 `RESOLVED(Node)`，generic scan 放行；static / 非 Node 派生的 FAILED 由上游 `sema.expression_resolution` 持有，gate 去重不重复发；property initializer 中的 `$`/`%` 当前为 DEFERRED，由 generic scan 升级为 `Expression remains deferred ...` 形态 compile error（见 `frontend_node_literal_implementation.md`）。
...
仍被封口的表达式（如 property initializer 内保持 DEFERRED 的 `GetNodeExpression`）会正常收到各自的
`sema.compile_check`，不再存在 declaration 级短路。
...
`GetNodeExpression` 已从显式 compile-block 列表移除（函数体与已记录 lambda `RESOLVED(Node)` 放行，property-init DEFERRED 边界经 generic scan 升级，见 `frontend_node_literal_implementation.md`）。
...
`AssertStatement` 已从显式 compile-block 清单移除（compile gate 放行 + truthiness 归一化 + backend `gdcc_assert_failed` 闭环）。
...
- `FrontendCompileCheckAnalyzerTest`
  - 显式 AST compile-block 已清空（GetNode / Preload / Array / Dictionary / Cast / TypeTest / Conditional 均不在 intercept 列表）；GetNode 的剩余边界（property initializer DEFERRED）由 generic published-fact scan 升级
  ...
  - for route-aware compile policy：`RANGE_CALL` / `INT_SHORTHAND` 凭已注册 contract 放行（无 `sema.compile_check`，且释放后进入 body 重扫其中的 published facts——get-node 在函数体已 `RESOLVED(Node)` 放行，不再作为封口锚点）；`GENERIC_VARIANT` 在 statement root 发 route-not-ready blocker 且说明缺少 lowering route；同一 ForStatement anchor 已有 upstream error 时不补发同级 `sema.compile_check`
  - lambda compile gate：`sig.connect(func(): ...)` 直接实参放行（无 `compile_check`/unsupported）；已记录 lambda body 内 `$`/`%` 与 `preload`、`assert` 均 compile-ready，递归扫描不发 blocker；property-initializer / parameter-default 未记录 lambda 保持 fail-closed（上游 unsupported owner 持有，不补发 `compile_check`）；lambda body 内 `match` 六 route 全部放行，不再由 `unsupported_binding_subtree` 持有
...
- `GetNodeExpression` 的函数体与已记录 lambda 路径已完成 sema（`RESOLVED(Node)` + static/非 Node `FAILED`）、CFG/body lowering（`literal_node_path` + `self` 上溯 `Node` 的 `assign` + `call_method "get_node"`）与 backend ENGINE dispatch 闭环，compile gate 不再保留表达式级显式拦截（property initializer DEFERRED 边界仍由 generic scan 封口）；`PreloadExpression` 已完成 sema 字面量校验、compile gate 放行、CFG/body lowering（ResourceLoader singleton 调用对改写）与 backend 闭环；`assert` 已完成 compile gate 放行、CFG/body lowering 与 C backend 闭环；`ArrayExpression` / `DictionaryExpression`、`TypeTestExpression`、`CastExpression` 与 `ConditionalExpression` 已完成 shared semantic、CFG/body lowering 与 backend 闭环；`for` 已注册 route 的 CFG/lowering 已落地，compile gate 为 route-aware policy（registry 已注册 route 放行，`OBJECT_CUSTOM` 等未注册 route 发 route-not-ready blocker）；已记录 `LambdaExpression` 的 shared semantic、`construct_lambda` lowering 与 C backend 已闭环，compile gate 按 published plan 放行并递归扫描 body
