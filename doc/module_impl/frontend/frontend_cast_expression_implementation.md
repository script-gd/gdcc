...
- 当前仍被显式 intercept 的表达式（`PreloadExpression`、`GetNodeExpression`）以及 `assert` / route-aware `ForStatement` 仍被正确封口。`ArrayExpression` / `DictionaryExpression` 与 `ConditionalExpression` 已离开 intercept（见 `frontend_container_literal_implementation.md`、`frontend_conditional_expression_plan.md`）。
