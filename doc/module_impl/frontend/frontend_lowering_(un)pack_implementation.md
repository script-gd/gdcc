...
- source 是 `int`，target 是 `float`
  - frontend shared semantic 通过 `ALLOW_WITH_PRIMITIVE_CAST` 接受该边界
  - body lowering 必须显式插入 `CallIntrinsicInsn(..., "c_int_to_float", ...)`，并返回新的 `float` temp slot
...
- `ALLOW_WITH_PRIMITIVE_CAST` 表示 source-level 边界合法，但 LIR 需要显式 primitive conversion materialization；当前唯一成员是 `int -> float`，对应 `call_intrinsic "c_int_to_float"`
...
- primitive cast：分配 target-typed 新 temp，并追加 backend-owned `CallIntrinsicInsn`
...
### 4.5 primitive cast boundary 合同

`int -> float` 是 ordinary typed boundary conversion，不是 builtin constructor special route：
...
- 除 `int -> float` 这一已纳入矩阵的 primitive cast 外，其它 widened scalar conversion
