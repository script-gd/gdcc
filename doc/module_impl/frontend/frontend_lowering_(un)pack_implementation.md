...
### 4.5 intrinsic cast boundary 合同

`int -> float` 与同维度 `Vector*i -> Vector*` 是 ordinary typed boundary conversion，不是 builtin constructor special route：

- `var x: float = 1`、`var v: Vector3 = Vector3i(1, 2, 3)`、assignment、return、call fixed argument、vararg 与 subscript key/index 都必须消费 shared helper 的 `ALLOW_WITH_INTRINSIC_CAST` decision
- lowering 不得在各 consumer 中硬编码这些转换；唯一物化入口仍是 `FrontendBodyLoweringSession.materializeFrontendBoundaryValue(...)`
