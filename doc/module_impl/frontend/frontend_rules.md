...
- 协程与 signal-based coroutine 当前不在 frontend semantic MVP 范围内；`await` 仍未闭环。`Signal.emit(...)` / `Signal.connect(...)` / `Signal.disconnect(...)` 已走既有 builtin `CallMethodInsn` 路径；Object/self method-reference → Callable 已闭环，builtin/static/utility/lambda method-reference 仍被拒绝。
