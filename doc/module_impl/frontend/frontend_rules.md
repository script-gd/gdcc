...
- `_field_init_`、`_field_getter_`、`_field_setter_` 是 compiler-owned synthetic property helper 前缀；`_lambda_` 是 compiler-owned synthesized lambda function 前缀（lambda 计划 §3.7，阶段 E 起并入 `RESERVED_PREFIXES`）。source class member 一旦以这些前缀开头，skeleton phase 必须发出清晰的 `sema.class_skeleton` 并跳过该 member subtree，而不是等到 lowering/backend 再因 helper 名冲突抛异常。
