...
- `@export*` 家族（裸 `export` 与 `export_range` / `export_enum` / `export_flags*` / `export_file*` / `export_dir` / `export_global_*` / `export_multiline` / `export_placeholder` / `export_exp_easing` / `export_color_no_alpha` / `export_node_path`）的合同固定为：参数只接受字面量（含数值一元 `+`/`-`），经共享 helper `FrontendExportAnnotationSupport` 求值并编码为 Godot `hint_string` 写入 property annotation value；挂载位置（仅 class `var` property）、static 拒绝、参数与类型兼容校验统一由 `sema.annotation_usage` owner 发出，skeleton 对 malformed 参数只 retention 不发诊断。容器声明类型上的所有 export variant 一律拒绝（不做 Godot 的 Array peel），裸 `export` 不受限；`export_category` / `export_group` / `export_subgroup` / `export_storage` / `export_custom` / `export_tool_button` 仍走 `sema.unsupported_annotation`。脚本 enum 类型标注 property 上的裸 `@export`（无任何 `export_*` variant）由 skeleton 生成 `capitalize(成员名):值` hint_string 覆写 `"export"` value，backend 映射 `PROPERTY_HINT_ENUM`；显式 variant 与求值失败的枚举（Variant 回退）维持原行为。
