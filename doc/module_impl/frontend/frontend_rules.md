...
- `ConditionalExpression`、`ArrayExpression`、`DictionaryExpression`、`PreloadExpression`、`GetNodeExpression`、`CastExpression`、`TypeTestExpression` 当前属于 frontend 已识别但 **compile gate 仍拦截** 的 temporary compile intercept；其中 `TypeTestExpression` 的 shared semantic 已发布 `RESOLVED(bool)` 与 `typeTestTargets()`，body lowering 已可发射统一 `is_instance_of` / 常量 bool（见 `frontend_is_type_test_implementation_plan.md` Phase 1–2），但 **Phase 4 之前** gate 不得解封。其余 intercept 表达式仍可走 deferred/unstable facts，但 compile-only gate 必须在进入 lowering 前最终封口。compound assignment 已不再属于这组 temporary compile intercept。
