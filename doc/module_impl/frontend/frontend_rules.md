...
- 协程与 signal-based coroutine 当前不在 frontend semantic MVP 范围内；`await` 仍未闭环（设计合同已由 `frontend_await_minicoro_plan.md` 冻结，frontend 落地见其第六/七步，compile gate 第八步解封）。`Signal.emit(...)` / `Signal.connect(...)` / `Signal.disconnect(...)` 已走既有 builtin `CallMethodInsn` 路径；Object/self、非 Dictionary builtin 实例、GDCC/engine 静态与 utility 值引用 → Callable 已闭环；已记录 lambda → `Callable` 经 `construct_lambda` 已闭环。builtin type-meta / `Dictionary` key / `Node.new` 仍被拒绝。长合同见 `frontend_signal_support.md`。
