...
- 协程与 signal-based coroutine 当前不在 frontend semantic MVP 范围内；`await` 仍未闭环。`Signal.emit(...)` 已走既有 builtin vararg `CallMethodInsn` 路径；`connect` / `disconnect` 与 method-reference → Callable 仍未闭环。
