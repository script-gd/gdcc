...
- source function 参数默认值已进入 shared island 语义与 compile surface（见 `frontend_parameter_default_plan.md`）：默认值表达式只可见字面量/常量/枚举/类型/单例/builtin 构造器/utility/global/static 调用，instance 方法额外可见 `self` 与实例成员；constructor/`_init`/lambda 参数默认值、默认值内 `await`、对参数/局部变量/capture 的引用仍 fail-closed。
