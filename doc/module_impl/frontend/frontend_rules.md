...
- `ConditionalExpression`、`ArrayExpression`、`DictionaryExpression`、`PreloadExpression`、`GetNodeExpression`、`CastExpression` 当前属于 frontend 已识别但 **compile gate 仍拦截** 的 temporary compile intercept。`TypeTestExpression` 已从这组 intercept 中移除：shared semantic 发布 `RESOLVED(bool)` 与 `typeTestTargets()`，body lowering 发射统一 `is_instance_of` / 常量 bool，backend `IsInstanceOfInsnGen` 分派 + runtime helpers 已落地（见 `frontend_is_type_test_implementation.md`）。`CastExpression` 的 shared semantic 已发布 `RESOLVED(targetType)` 与 `sema.unsafe_cast` / `sema.type_check` diagnostic（见 `frontend_cast_expression_implementation_plan.md` Phase 1），但 body lowering / LIR / backend 尚未接通，故 compile gate 仍必须封口。其余 intercept 表达式仍可走 deferred/unstable facts，但 compile-only gate 必须在进入 lowering 前最终封口。compound assignment 已不再属于这组 temporary compile intercept。
