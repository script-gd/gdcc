...
- compile-only gate 只允许扫描未来 lowering 会消费的 compile surface：supported executable body 与 supported property initializer island；不得重新深入 parameter default、lambda、`match`、block-local `const` 或 skipped subtree。`ForStatement` 已进入 shared semantic，compile-only gate 按 route-aware policy 处理：已注册 lowering contract 的 route（当前 `RANGE_CALL` / `INT_SHORTHAND`）放行并进入 body 重扫 semantic facts，未注册 contract 的 route 在 statement root 发 route-not-ready blocker；完整 CFG/lowering 闭环仍待阶段 G/H。
