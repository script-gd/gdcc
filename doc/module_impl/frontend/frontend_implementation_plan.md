...
- 若处于类成员的无 base 解析，先按当前 callable 是否为 static 构造 `ResolveRestriction`，再调用 restriction-aware `Scope` API；
- 若 `Scope` 返回 `FOUND_BLOCKED`，则直接形成“当前上下文非法访问该绑定”的语义结论，不继续回退 outer/global 的同名绑定；
- 其中未限定类成员的允许集合当前冻结为：
  - `static context`：允许 class const / static property / static method
  - `instance context`：允许 class const / instance property / instance method / static property / static method
- `type-meta` 也走统一的 `Scope.resolveTypeMeta(..., restriction)` 形状，但当前协议明确规定：
  - 对现有 `ScopeTypeMetaKind` 集合只允许返回 `FOUND_ALLOWED` / `NOT_FOUND`
  - 当前 `type-meta` lookup 不产生 `FOUND_BLOCKED`
  - `TYPE_META` 的后续合法性由 binder 在 static access / constructor / `load_static` 等消费阶段负责，而不是由 lookup 本身负责
...
- overload 选择（见 `BackendMethodCallResolver`）
...
对照后端实现（`BackendPropertyAccessResolver`、`IndexLoadInsnGen/IndexStoreInsnGen`），前端建议在语义层区分三类访问。
...
- 已落地：
  ...
  - `dev.superice.gdcc.exception.FrontendSemanticException`
- 已实现能力：
